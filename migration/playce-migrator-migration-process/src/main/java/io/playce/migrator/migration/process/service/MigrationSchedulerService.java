...
@Slf4j
public class MigrationSchedulerService {
    private final SchedulerConfig schedulerConfig;
    private final TaskRunnerComponent taskRunnerComponent;

    @Scheduled(fixedDelayString = "#{migrationJobConfig.fixed.fixedDelay * 1000}", initialDelayString = "#{migrationJobConfig.fixed.initDelay * 1000}")
    public void runSchedule() {
        SchedulerToggle toggle = schedulerConfig.getToggle();
        if(!toggle.isEnableMigration()) return;

        taskRunnerComponent.runTaskService(TaskType.Migration);
    }
}